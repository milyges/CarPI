...
WndMain::WndMain(QWidget *parent) : QMainWindow(parent), _ui(new Ui::WndMain) {
    ...
    action = _mainMenu->addAction("Navitgation Menu");
    connect(action, SIGNAL(triggered(bool)), this, SLOT(_showNavitMenu()));

    _viewMenu = _mainMenu->addMenu("View");
    ...
    action = _mainMenu->addAction("Return");
    connect(action, SIGNAL(triggered(bool)), this, SLOT(_menuHide()));

    _showMapAction = _viewMenu->addAction("Map");
    ...
    _showDashboardAction = _viewMenu->addAction("Dashboard");
    ...
    action = _systemMenu->addAction("Shutdown");
